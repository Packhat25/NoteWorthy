...
namespace NoteWorthy.Panels
{
    public partial class statistics : basePanel
    {
        ...
        {
            DataTable dt = dbHelper.GetGenreDistribution();
            ...
            chartGenre.Series["Genres"].ChartType = SeriesChartType.Pie;
            ...
            chartGenre.Series["Genres"]["PieLabelStyle"] = "Outside";

            chartGenre.Series["Genres"]["PieLineColor"] = "Black";

            foreach (var genre in genreCounts)
            {
                int pointIndex = chartGenre.Series["Genres"].Points.AddXY(genre.Key, genre.Value);
                chartGenre.Series["Genres"].Points[pointIndex].LegendText = genre.Key;
            }

            int total = genreCounts.Values.Sum();
            lblTotalBookmarks.Text = $"Total bookmarks: {total}";
        }
        ...
        private void statistics_Load(object sender, EventArgs e)
        {
            ...
            lblTotalUsers.Text = $"Total Users: {totalUsers-1}";
        }
    }
